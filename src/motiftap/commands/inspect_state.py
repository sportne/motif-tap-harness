...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Print widgets from a Motif Tap latest-state.json file."
    )
    parser.add_argument("state_file")
    parser.add_argument(
        "--contains", default=None, help="Only show paths containing this substring"
    )
    ...
    for w in rows:
        ...
        print(
            f"{w.path[:path_width]:{path_width}}  {w.klass[:22]:22s}  {geometry:18s}  {','.join(flags)}"
        )
